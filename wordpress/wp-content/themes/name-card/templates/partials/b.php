...
if (isset($_GET['download_vcard']) && !empty($user_id) && !empty($company)) {
    if ($card) {
        $address .= esc_attr($card->address) . "rn"; // Dùng $card->address thay vì $card['address']
        $vcard = "BEGIN:VCARDrn";
        $vcard .= "VERSION:3.0rn";
        $vcard .= "FN:" . esc_attr($card->name) . "rn";
        $vcard .= "EMAIL:" . esc_attr($card->email) . "rn";
        $vcard .= "TEL:" . esc_attr($card->phone) . "rn";
        ...
        if (!empty($card->image)) {
            ...
            if (file_exists($image_path)) {
                ...
                $vcard .= "PHOTO;ENCODING=b;TYPE=JPEG:$base64_imagern"; // Sử dụng TYPE=JPEG cho định dạng ảnh JPEG
            }
        }
        ...
        if (!empty($address)) {
            $vcard .= "ADR;TYPE=home:;;$address;;;;rn";
        }
        ////
        // Thêm URL Google Maps nếu địa chỉ không rỗng
        if (!empty($address)) {
            $map_query = urlencode($address);
            $vcard .= "URL:https://www.google.com/maps/search/?api=1&query=$map_queryrn";
        }

        if (!empty($company)) {
            $vcard .= "ORG:" . esc_attr($company->name) . "rn"; // Tên công ty

        }

        if (!empty($user) && !empty($user->jobname)) {
            $vcard .= "TITLE:" . esc_attr($user->jobname) . "rn"; // Thêm jobname từ company
        }
